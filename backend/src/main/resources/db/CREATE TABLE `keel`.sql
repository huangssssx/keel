...
AND NOT EXISTS (
    ...
    AND ur.role_id = r.id
); '超级管理员', 'ROLE_ADMIN', '系统超级管理员', 1
WHERE NOT EXISTS (-- 创建用户角色关联表
CREATE TABLE IF NOT EXISTS `sys_user_role` (
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `role_id` bigint NOT NULL COMMENT '角色ID',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
AND NOT EXISTS (
    ...
    WHERE ur.user_id = u.id 
    AND ur.role_id = r.id
); 1 FROM `sys_role` WHERE `role_code` = 'ROLE_ADMIN');
...
AND NOT EXISTS (
    ...
    WHERE ur.user_id = u.id 
    AND ur.role_id = r.id
); 'admin', '$2a$10$ow7aLYZbIBim4VkxZ.i1auYcgxHyX6NJsVzLUKxoaKkZqHDFCCPHO', '管理员', 1
WHERE NOT EXISTS (-- 创建用户角色关联表
CREATE TABLE IF NOT EXISTS `sys_user_role` (
  `user_id` bigint NOT NULL COMMENT '用户ID',
  `role_id` bigint NOT NULL COMMENT '角色ID',
...
AND NOT EXISTS (
    ...
    WHERE ur.user_id = u.id 
    AND ur.role_id = r.id
); 1 FROM `sys_permission` WHERE `permission_code` = 'system:manage');
